...
#define WALLY_FN_33_A(F, N) template <class O> inline int F(uint32_t i321, uint32_t i322, O **out) { \
        return ::N(i321, i322, out); \
}
...
#define WALLY_FN_P(F, N) template <class P1> inline int F(const P1 &p1) { return ::N(WALLYP(p1)); }
...
#define WALLY_FN_P3_S(F, N) template <class P1, class O> inline int F(const P1 &p1, uint32_t i321, size_t * written) { \
...
#define WALLY_FN_PP3_BS(F, N) template <class P1, class P2, class O> inline int F(const P1 &p1, const P1 &p2, size_t * written, O & out, size_t offset = 0) { \
        return ::N(WALLYP(p1), WALLYP(p2), WALLYO(out), written); \
}

#define WALLY_FN_PP_BS(F, N) template <class P1, class P2, class O> inline int F(const P1 &p1, const P1 &p2, size_t * written, O & out, size_t offset = 0) { \
        return ::N(WALLYP(p1), WALLYP(p2), WALLYO(out), written); \
}
...
WALLY_FN_33_A(tx_witness_stack_init_alloc, wally_tx_witness_stack_init_alloc)
