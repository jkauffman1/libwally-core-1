...
class TxTests(unittest.TestCase):
    ...
    def do_test_tx(self, sighash, index_, flags):
        ...
        tx_add_input(tx, tx_input_init(txhash, 1, seq-1, script+b'\x51', None))
        tx_add_input(tx, tx_input_init(txhash, 2, seq-2, script+b'\x51\x51', None))
        tx_add_input(tx, tx_input_init(txhash, 3, seq-3, script+b'\x51\x51\x51', None))
        tx_add_output(tx, tx_output_init(55, out_script))
        tx_add_output(tx, tx_output_init(54, out_script+b'\x51'))
        tx_add_output(tx, tx_output_init(53, out_script+b'\x51\x51'))
